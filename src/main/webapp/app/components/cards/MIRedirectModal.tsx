...
export default class MIRedirectModal extends Component<MIRedirectModalProps, MIRedirectModalState> {
  ...
  componentDidUpdate(prevProps: Readonly<MIRedirectModalProps>, prevState: Readonly<MIRedirectModalState>, snapshot?: any) {
  ...
  getImdbLink() {
    return `https://www.imdb.com/title/${this.props.movie?.imdbId}/`;
  }

  getTmdbLink() {
    return `https://www.themoviedb.org/movie/${this.props.movie?.tmdbId}/`;
  }
  ...
  render() {
    return (
      <CModal
        ...
        onClose={this.close}
      >
        <CModalHeader closeButton><h5 className="text-warning">Do you want to see more?</h5></CModalHeader>
        <CModalBody>
          <MIMovieCard type={MIMovieCardType.MostVote} entity={this.props.movie}/>
        </CModalBody>
        <CModalFooter>
          <CButton style={{display:this.props.movie?.imdbId?"block":"none"}} color="warning" onClick={()=>window.location.href=this.getImdbLink()}><CIcon className={"btn-icon text-dark"} size={"xl"} name={"cib-imdb"}/> View on IMDb</CButton>{' '}
          <CButton style={{display:this.props.movie?.tmdbId?"block":"none"}} className="btn-tumblr text-value-md" onClick={()=>window.location.href=this.getTmdbLink()}><CIcon className={"btn-icon"} size={"xl"}  content={tmdbSvg}/> View on TMDb</CButton>{' '}
          <CButton color="secondary" onClick={this.close}> Cancel </CButton>
        </CModalFooter>
      </CModal>
    )
  }
