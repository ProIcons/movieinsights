...
export const ACTION_TYPES = {
  SEARCH_VOTES: 'vote/SEARCH_VOTES',
...
export default (state: VoteState = initialState, action): VoteState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_VOTES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_VOTES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_VOTES):
...
export const getSearchEntities: ICrudSearchAction<IVote> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_VOTES,
  payload: axios.get<IVote>(`${apiSearchUrl}?query=${query}`),
});
